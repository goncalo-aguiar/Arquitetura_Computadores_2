...
void _int_(32) isr_UART2(void)
{
    ...
    if(IFS1bits.U2RXIF == 1)// if U1TXIF is set
    {  
        ...
        if(){  // If reception buffer is not full (e.g. count < BUF_SIZE) then 
            rxb.count++;       //   increment "count" variable 
        }
        else{// Else 
            txb.tail = (txb.tail + 1) & INDEX_MASK; //   increment "head" variable (discard oldest character)
        }
